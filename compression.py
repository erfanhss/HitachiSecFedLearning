...
def random_matrix(p, s, w, seed):
    counts = tf.cast(
        tf.random.stateless_normal([1], seed, mean=s * w * p * 2, stddev=np.sqrt(s * w * 2 * p * (1 - 2 * p))),
        dtype=tf.int32)
...
def master():
    ...
    savestr = "num_peers_" + str(num_peers) + "_robust_" + str(robust) + "_compression_" + str(
        compression) + "_resiliency_" + str(resiliency) + "_compression_rate_" + str(r)
    init = time.time()
    ...
    for iteration in range(iterations):
        ...
        if compression == 1:
            ...
            update = tf.sparse.sparse_dense_matmul(tf.sparse.transpose(phi),
                                                   tf.convert_to_tensor(update, dtype=tf.float32))
        ...
        print("-----------------------------------")
        np.save('accuracy_' + savestr, np.array(accuracy))
        np.save('loss_secure' + savestr, np.array(loss))
...
def client():
    ...
    for iteration in range(iterations):
        ...
        if compression == 1:
            ...
            compressed = grad*learning_rate
        GoWrappers.client_phase2(compressed.numpy().reshape(-1), pk, shamir_share, id, server_Address, robust,
                                 log_degree, log_scale, resiliency)
